...
def get_full_gradient_l2_norm(pruned_model, gradient_dict):
    total_l2_gradient_list = torch.Tensor([]).cuda()

    for name, gradient in gradient_dict.items():
        if 'bn' not in name and 'bias' not in name:
            origin = torch.zeros(gradient.shape).cuda()
            L2_norm = torch.Tensor([torch.dist(origin, gradient, 2).item()]).cuda()
            total_l2_gradient_list = torch.cat((total_l2_gradient_list, L2_norm))
    
    total_l2_norm = torch.dist(total_l2_gradient_list, torch.zeros(total_l2_gradient_list.shape).cuda(), 2)
    return total_l2_norm.item()


def get_class_gradients_list(pruned_model, class_gradient_dict, gradient_dict):
    gradient_list = torch.Tensor([]).cuda()

    for name, gradient in gradient_dict.items():
        if 'bn' not in name and 'bias' not in name:
            if name not in class_gradient_dict:
                class_gradient_dict[name] = []
            
            class_gradient_dict[name].append(gradient)
    
    return class_gradient_dict
...
def get_gradient_mean_dict(pruned_model, trainloader):
    ...
    for i, (inputs, targets) in enumerate(tqdm(trainloader)):
        ...
        inputs = inputs.cuda()
        class_gradient_norms = []
        class_gradient_dict = dict()
        ...
        for class_index in range(0, 10):
            ...
            class_gradient_dict = get_class_gradients_list(pruned_model, class_gradient_dict, model_grad_dict) 
            l2_norm = get_full_gradient_l2_norm(pruned_model, model_grad_dict)
            class_gradient_norms.append(l2_norm)

        max_class = np.argmax(class_gradient_norms)
        min_class = np.argmin(class_gradient_norms)

        for name, class_gradient_list in class_gradient_dict.items():
            if 'layer3' in name:
                continue

            if name not in gradient_divergence_dict:
                gradient_divergence_dict[name] = torch.zeros(class_gradient_list[min_class].shape).cuda()
            
            max_min = calculate_score(class_gradient_list[min_class], class_gradient_list[max_class])
            ...
            #     max_min[torch.isinf(max_min)] = 100000000000

            gradient_divergence_dict[name] += max_min
...
def get_gradient_covariance_dict(pruned_model, trainloader, gradient_mean_dict):
    ...
    for i, (inputs, targets) in enumerate(tqdm(trainloader)):
        ...
        for name, class_gradient_list in class_gradient_dict.items():
            ...
            x_mean = max_min - gradient_mean_dict[name]
