...
if __name__ == '__main__':
    ...
    encoder = autoencoder.autoencoder(640)
    encoder_checkpoint = torch.load("./gradients/test_resnet_autoencoder/gmm/level_0/autoencoder_checkpoint.pth")
    ...
    gradient_encoder_score_dict = dict()
    ...
    for i, (inputs, targets) in enumerate(tqdm(testloader)):       
        ...
        for name, gradient in class_gradient_list[min_class].items():
            ...
            if 'linear.weight' in name:
                ...
                output = encoder(flatten_gradient)
                mse_loss = MSE(output, flatten_gradient)
                score = mse_loss.item()

                if name not in gradient_encoder_score_dict:
                    gradient_encoder_score_dict[name] = []
                
                gradient_encoder_score_dict[name].append(score)

    directory = './gradients/{name}/{form}/level_{level}/'.format(name="test_autoencoder_scores", form=args.saved_form, level=args.pruning_level)
    if not os.path.exists(directory):
        os.makedirs(directory)

    torch.save(gradient_encoder_score_dict, directory + '{dataset}_{name}.pth'.format(dataset=args.dataset, name="score"))
    print("\nAutoencoder scores saved in {directory}{dataset}_{name}.pth".format(directory=directory, dataset=args.dataset, name="score"))