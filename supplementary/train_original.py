...
parser.add_argument('--model-arch', default='resnet_20', type=str, help='model architecture')
...
parser.add_argument('--pruning_levels', default=0, type=int,
...
def validate(val_loader, model, criterion, epoch):
    ...
    for i, (input, target) in enumerate(val_loader):
        input = input.cuda()
        target = F.one_hot(target, 10).long().cuda()
    ...
    print('---------------> Completed Validation with Accuracy {top1.avg:.3f} <---------------'.format(top1=top1))
    return top1.avg
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        target = target.type_as(nat_output)
        ...
        if i % args.print_freq == 0:
            print("Epoch[{epoch}] | Iteration[{iter}]: {loss}".format(epoch=epoch, iter=i, loss=loss))
